    ...
    .then((new_job) => {
      ...
      JobApplicant.find({
        ...
        .exec((err, applicants) => {
          ...
          //if applicant.length<5, call new applicant with parameters skill set only
          let idDetails = applicants.map((item) => {
            return item.userId;
          });
          User.find({ _id: { $in: idDetails } })
            .collation({ locale: "en" })
            .exec((err, applicantUser) => {
              if (err) {
                // Handle error
                console.error(err);
                return res.status(500).json({ error: "Internal Server Error" });
              }
              applicantEmail = applicantUser.map((item) => {
                return item.email;
              });
              applicantEmail.forEach((element) => {
                console.log(element);
                console.log("forEach");
                sendMail({
                  from: '"Sachin" <sachinstha977@gmail.com>',
                  to: element,
                  subject: "New Job In Your Locality",
                  html: `<h1>A recruiter is hiring for a job that matches your skill sets.<h1>
            
                            <a href="http://localhost:3000/jobs/${new_job?._id}">Click this link to apply</a>`,
                });
              });
...
router.get("/jobs/:id", jwtAuth, (req, res) => {
  const findParams11 = { _id: mongoose.Types.ObjectId(req.params.id) }; // Assuming mongoose is imported and used for ObjectId conversion
  ...
  Job.aggregate(arr)
    .then((job) => {
      ...
      console.log("Getting Particular job...");
      console.log(job);
      res.json(job);
    })
    .catch((err) => {
                      ...
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                          ...
                          application
                            .save()
                            .then((newApplication) => {
                              User.findById(newApplication.recruiterId).then(
                                (recruterUser) => {
                                  sendMail({
                                    from: '"Sachin" <sachinstha977@gmail.com>',
                                    to: recruterUser.email,
                                    subject: `A new Application fro Your Job ${job?.title}`,
                                    html: `<h1>Good News, You have got a new application for your job.<h1>
                          
                                          <a href="http://localhost:3000/job/applications/${newApplication.jobId}">Click to view all Applicants</a>`,
                                  });
                                }
                              );
                              // User.findById(newApplication.userId).then(
                              //   (applicantUser) => {
                              //     console.log(
                              //       "sercing applicant of the applied job"
                              //     );
                              ...
                              //       subject:
                              //         "Great, You have applied for new Job",
                              //       html: `<h1>Please follow the link below to see your progess.<h1>

                              //             <a href="http://localhost:3000/jobs/${newApplication.jobId}">Click to view all Applicants</a>`,
                              //     });
                              //   }
                              // );
  ...
  if (user.type === "recruiter") {
    if (status === "accepted") {
                        ...
                        if (status === "accepted") {
                          Job.findOneAndUpdate(
                            ...
                            .then(() => {
                              ...
                              User.findById(updatedApplication.userId).then(
                                (applicantUser) => {
                                  console.log(
                                    "sercing applicant of the applied job"
                                  );
                                  ...
                                  sendMail({
                                    ...
                                    to: applicantUser.email,
                                    subject:
                                      "Congratulations, You are accepted for the following Job",
                                    html: `<h1>Please follow the link below to see your progess.<h1>

                                          <a href="http://localhost:3000/jobs/${updatedApplication.jobId}">Click to view all Applicants</a>`,
                                  });
                                }
                              );
                              res.json({
      ...
      Application.findOne({
        ...
        User.findById(application.userId).then((applicantUser) => {
          sendMail({
            from: '"Sachin" <sachinstha977@gmail.com>',
            to: applicantUser.email,
            subject: "Oops, You are not selected for the following Job",
            html: `<h1>Please follow the link below for more details.<h1>
  
                      <a href="http://localhost:3000/jobs/${application.jobId}">Click to view all Applicants</a>`,
          });
        });
        res.json({
